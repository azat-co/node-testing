...
describe('Node University', () => {
  ...
  it('has View All FREE Courses link', async () => {
    ...
    expect(a).to.be.ok
    await a.click()
    await driver.wait(until.titleIs('Node University: Courses on Node.js, React and JavaScript'), 1000)
    expect(await driver.getCurrentUrl()).to.eql('https://node.university/courses/category/free')
  })
  it.only('has AWS course', async ()=>{
    ...
    expect(div).to.be.ok
    div.click()
    driver.wait(until.titleIs('Node in Production Using Docker and AWS | Node University: Courses on'), 1000)
